...
namespace lolita
{
RgbPixel convolutionElement(Image& mat, uint32_t row, uint32_t column, Mat<double>& kernel);
RgbPixel traverse(Image& mat, uint32_t row, uint32_t column, uint32_t radius, std::function<RgbPixel(std::vector<RgbPixel>&)> callback);
